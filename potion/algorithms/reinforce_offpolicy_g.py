...
from potion.algorithms.ce_optimization_g import optimize_behavioural, get_alphas, var_mean
...
def reinforce_offpolicy_g(
    ...
    if ce_use_offline_data:
        ...
        offline_batches = [generate_batch(env, policy, horizon, batchsize, 
                                        ...
                                        seed=seed,
                                        n_jobs=parallel)]
        njt_batches = [generate_batch(env, policy, horizon, batchsize, 
                                        action_filter=action_filter,
                                        seed=seed,
                                        n_jobs=parallel)]
        if debug_logger is not None:
    ...
    for it in range(iterations):
        ...
        if verbose:
            print('\nIteration ', it)
        log_row, offline_policies, offline_batches, njt_batches = reinforce_offpolicy_step_g(
            **algo_params,
...
def reinforce_offpolicy_step_g(
        env, policy, horizon, behavioural_policies, offline_policies, offline_batches,  njt_batches, *,
        action_filter=None,
        batchsize=100, 
        baseline='avg',
        biased_offpolicy=True,
    ...
    if offline_policies:
        ...
        try:
            optimize_behavioural(
                behavioural_policies[0], env, policy, offline_policies, offline_batches, njt_batches, 
                estimator = estimator,
                baseline = baseline,
    ...
    if ce_batchsizes is not None:
        for i,ce_batchsize in enumerate(ce_batchsizes):
            ...
            njt_batches_genned.append(generate_batch(env, behavioural_policies[i], horizon, njt_batchsize, 
                               action_filter=action_filter, 
                               seed=seed, 
                               n_jobs=False))
            ...
            try:
                ...
                optimize_behavioural(
                    behavioural_policies[i+1], env, policy, behavioural_policies[:i+1], behavioural_batches, njt_batches_genned,
                    estimator = estimator,
                    baseline = baseline,
                    optimize_mean = ce_optimize_mean,
                    optimize_variance = ce_optimize_variance,
    ...
    njt_batches_n.append(generate_batch(env, behavioural_policies[-1], horizon, batchsize,
                       action_filter=action_filter, 
                       seed=seed, 
                       n_jobs=False)
    )
    ...
    if defensive_batch > 0:
        ...
        """
        njt_batches_n.append(generate_batch(env, policy, horizon, defensive_batch,
                            action_filter=action_filter, 
                            seed=seed, 
                            n_jobs=False)
        )
    ...
    if biased_offpolicy:
        ...
        if behavioural_batches:
            ...
            njt_batches_n += njt_batches_genned
    ...
    if ce_batchsizes is not None:
        ...
        log_row["Batch_total"] += njt_batchsize
    ...
    if offline_policies:
        ...
        next_njt_batches = njt_batches_n
    else:
        next_offline_policies = []
        next_offline_bathces  = []
        next_njt_batches = []
    return log_row, next_offline_policies, next_offline_bathces, next_njt_batches
...
if __name__ == '__main__':
    ...
    res = reinforce_offpolicy_g(env, policy, env.horizon,
                 action_filter = None,
                 batchsize = 100, 
                 baseline = 'peters',
                 biased_offpolicy = True,
