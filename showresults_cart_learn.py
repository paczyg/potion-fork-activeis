...
dir_results = 'cartpole/results/paper'


on_batchsize_list           = [10, 10, 10, 10, 10]
onbs100_batchsize_list      = [100, 100, 100, 100, 100]
off_batchsize_list          = [80, 66, 53, 40, 27 ]
off_defensive_batch         = [ 0,  14,  27, 40, 53]
...
my_ce_batchsize_list        = [10, 10, 10, 10, 10]
...
for i in range(len(on_batchsize_list)):
    ...
    exp_on       = f"{dir_results}/onpolicy_bs10/batchsize{on_batchsize_list[i]}"
    exp_onbs100  = f"{dir_results}/onpolicy_bs100/batchsize{onbs100_batchsize_list[i]}"
    exp_off      = f'{dir_results}/offpolicy/ce_batchsizes{ce_batchsize_list[i]}batchsize{off_batchsize_list[i]}defensive_batch{off_defensive_batch[i]}'
    #exp_stormpg  = f"{dir_results}/my/init_batchsize{stormpg_init_batchsize_list[i]}mini_batchsize{stormpg_mini_batchsize_list[i]}"
    exp_my = f"{dir_results}/my_bs10ceupdate8/ce_batchsizes{my_ce_batchsize_list[i]}defensive_batch{my_defensive_batch[i]}batchsize{my_batchsize_list[i]}"
    ...
    plt.figure()
    plt.title(f'Cartpole Defensive alpha = {my_defensive_batch[i]/10}')
   ...
   # Onpolicy bs100
    m, ci_lb, ci_ub = plot_utils.get_ci(mysuite, exp_onbs100, 'TestPerf')
    #print(m.shape)
    #print(len(mysuite.get_history(exp_on,0,'Batch_total')))
    plt.plot(np.cumsum([elem for elem in mysuite.get_history(exp_onbs100,0,'BatchSize')]), m, label='onpolicy batch size 100')
    plt.fill_between(np.cumsum([elem  for elem in mysuite.get_history(exp_onbs100,0,'BatchSize')]), ci_lb, ci_ub, alpha=.1)
    ...
    """
    plt.plot(np.cumsum([elem  for elem in  mysuite.get_history(exp_my,0,'Batch_total')]), m, label='DAIS-PG')
    plt.fill_between(np.cumsum([elem  for elem in  mysuite.get_history(exp_my,0,'Batch_total')]), ci_lb, ci_ub, alpha=.1)
